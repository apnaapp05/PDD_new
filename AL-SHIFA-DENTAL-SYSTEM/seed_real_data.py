...
db = SessionLocal()
try:
    # Get ID links (Assuming first hospital/doctor for now)
    hospital = db.query(Hospital).first()
    doctor = db.query(Doctor).first()
    
    # SAFETY: If no doctor/hospital exists, create defaults so the app works
    if not hospital:
        hospital = Hospital(name="Al-Shifa Dental", address="123 Main St", contact_number="555-0100")
        db.add(hospital)
        db.commit()
    
    if not doctor:
        # Assuming a user exists at ID 1, if not this might fail, but let's assume setup is done
        doctor = Doctor(user_id=1, hospital_id=hospital.id, specialization="General Dentist")
        db.add(doctor)
        db.commit()
