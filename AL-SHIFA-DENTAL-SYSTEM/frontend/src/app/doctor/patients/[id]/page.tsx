...
import { useParams } from "@/lib/next-navigation-shim";
...
import { User, FileText, Activity, Plus, Save, Upload, File as FileIcon, Download, Loader2, Sparkles } from "lucide-react";
...
export default function PatientDetails() {
  ...
  const fetchSummary = async () => {
    ...
    try {
      const res = await AgentAPI.getPatientSummary(Number(id));
      ...
      <Card className="bg-emerald-50 border-emerald-100">
        <CardHeader className="pb-2">
          <CardTitle className="text-base font-bold text-emerald-800 flex items-center gap-2">
            <Sparkles className="h-4 w-4 text-emerald-600" /> AI Clinical Summary
          </CardTitle>
        </CardHeader>
        <CardContent>
          {generatingSummary ? (
            <div className="flex items-center gap-2 text-sm text-emerald-600">
              <Loader2 className="h-3 w-3 animate-spin" /> Analyzing records...
            </div>
          ) : (
